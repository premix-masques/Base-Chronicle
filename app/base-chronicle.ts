...
const APP = {
  name: "Base Chronicle (Built for Base)",
...
async function readChainInfo(address: string) {
  const c = client();
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain monitoring, and detailed read-only blockchain analysis.";
  ...
  btnConnect.onclick = async () => {
    try {
      ...
      const stats = await readChainInfo(session.address);
